                ...
                for argument in event['arguments']:
                    ...
                    for i in range(k):
                        if re.search(argument['argument'],vocab[i]) :
                            if len(argument['argument'])<len(vocab[i]):
                                flag=1
                            else :
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            model.save_weights('best_model_add_balance_1.weights')
        print(
...
if __name__ == '__main__':
    ...
    evaluator = Evaluator()
    #model.load_weights('best_model_add.weights')
    model.fit_generator(
    ...
    predict_to_file('dev_data/dev.json','dev_data/dev.json', 'ee_pred_dev_add_s.json')
    predict_to_file('train_data/event_predict_lr_scheduler.jsonl', 'test1_data/test1.json', 'ee_pred_text_add_s.json')
