...
# 读取数据
train_data = load_data('train_data/balanced_train.json')
...
def viterbi_decode(nodes, trans):
    ...
    """
    labels = np.arange(num_labels).reshape((1, -1))  #np.arange()返回的是numpy.ndarray（type(np.arange(10)) == np.ndarray） 一维向量
...
def extract_arguments(text):
    ...
    #try:
    return {text[mapping[w[0]][0]:mapping[w[-1]][-1] + 1]: l for w, l in arguments}
   # except BaseException as e:
    #    for w, l in arguments:
   #         if w[-1]==len(tokens)-1:
    #            w[-1]=len(tokens)-2
     #           print(text, w, l, e)
    #    return {"": l for w, l in arguments}
...
# def predict_to_file(event_file,in_file, out_file):
#     """预测结果到文件，方便提交
#     """
...
#         for l in tqdm(fr):
#             w = linecache.getline(event_file, int(i) + 1)
#             l = json.loads(l)
#             w = json.loads(w)
#             event_list = []
#             for event in w['event_list']:
#                 text=event['event_type']+"|"+l["text"]
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            model.save_weights('best_model_add_balance.weights')
        print(
...
if __name__ == '__main__':
    ...
    evaluator = Evaluator()
    model.load_weights('best_model_add_balance.weights')
    ...
    predict_to_file('dev_data/dev.json','dev_data/dev.json', 'ee_pred_dev_add.json')
    predict_to_file('train_data/event_predict_lr_scheduler.jsonl', 'test1_data/test1.json', 'ee_pred_text_add.json')

    predict_to_file('dev_data/dev.json', 'ee_pred_dev_add_s1.json')
    predict_to_file('test1_data/test1.json', 'ee_pred_test1_add_s1.json')
    predict_to_file('test1_data/test2.json', 'ee_pred_test2_add_s1.json')
